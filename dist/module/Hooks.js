...
export let readyHooks = async () => {
    ...
    for (let el of scenes) { // Scene.collection
        ...
        for (let token of tokens) {
            ...
            if (tokenFlags?.itemId && !lootToken) {
                ...
                lootToken = await createLootToken(token.id, tokenFlags.itemId, false);
            }
        }
        scene = getGame().scenes?.active;
        ...
        for (let token of tokens) {
            ...
            let lootTokens = getLootToken({ itemId: tokenFlags?.itemId, tokenId: token.id });
    ...
    const items = getGame().items?.values();
...
const createDefaultFolders = async () => {
    ...
    let parentFolderId = getGame().settings.get(PICK_UP_STIX_MODULE_NAME, SettingKeys.parentItemFolderId);
    let folder = getGame().folders?.get(parentFolderId);
...
export const handleSocketMessage = async (msg) => {
    ...
    switch (msg.type) {
        ...
        case SocketMessageType.updateActor:
            await updateActor(getGame().actors?.get(msg.data.actorId), msg.data.updates);
            break;
...
//# sourceMappingURL=../maps/module/Hooks.js.map
