...
export class LootHud extends BasePlaceableHUD {
    ...
    getData(options) {
        ...
        if (!lootData) {
            //@ts-ignore
            error(`No valid LootToken instance found for token '${this.object?.document.id}' on scene '${this.object?.scene?.id}'`);
        }
        const data = {
            ...
            minPerceiveValue: this.object?.document.getFlag(PICK_UP_STIX_MODULE_NAME, PICK_UP_STIX_FLAG).minPerceiveValue ??
