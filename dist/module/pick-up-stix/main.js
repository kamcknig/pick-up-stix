...
import { collidedTokens, getActorCurrencyPath, getCurrencyTypes, getPriceDataPath, getQuantityDataPath, getWeightDataPath } from "../../utils.js";
import { LootToken } from "./loot-token.js";
import { ItemType, PickUpStixHooks, SocketMessageType } from "./models.js";
import { getCanvas, gmActionTimeout, SettingKeys } from "./settings.js";
...
export const normalizeDropData = async (data) => {
    ...
    const pack = data.pack ? game.packs.get(data.pack) : null;
    const id = data.id;
    ...
    if (!dropData.actor) {
        ...
        if (lootTokens.length === 0) {
            ...
            if (lootType === ItemType.ITEM) {
                ...
                const img = game.settings.get('pick-up-stix', SettingKeys.closeImagePath);
...
export const makeContainerApi = async (items, currency, position, tokenDataOverride = { width: 1, height: 1, closeImg: undefined, openImg: undefined }) => {
    ...
    const img = tokenDataOverride.closeImg || game.settings.get('pick-up-stix', SettingKeys.closeImagePath);
    return !!(await createLootToken({ ...tokenData, img }, {
        name: 'Container',
        img,
        type: ItemType.CONTAINER,
            ...
            'pick-up-stix': {
                'pick-up-stix': {
                    ...
                    container: {
                        ...
                        imageClosePath: img,
                        imageOpenPath: tokenDataOverride.openImg || game.settings.get('pick-up-stix', SettingKeys.openImagePath),
                        soundOpenPath: game.settings.get('pick-up-stix', SettingKeys.defaultContainerOpenSound),
                        soundClosePath: game.settings.get('pick-up-stix', SettingKeys.defaultContainerCloseSound),
                        loot: lootData
                    }
...
export const dropItemOnToken = async ({ dropData, targetTokenId }) => {
    ...
    const targetTokenFlags = targetToken.getFlag('pick-up-stix', 'pick-up-stix');
    const targetTokenItem = game.items.get(targetTokenFlags?.itemId);
    const targetTokenItemFlags = targetTokenItem?.getFlag('pick-up-stix', 'pick-up-stix');
    ...
    if (dropData.actor) {
        ...
        const item = (await game.items.get(id) ?? await game.packs.get(pack)?.getEntity(id));
        if (!item) {
...
export const addItemToContainer = async (data) => {
    ...
    if (game.user.isGM) {
        ...
        const containerItemFlags = duplicate(containerItem.getFlag('pick-up-stix', 'pick-up-stix'));
...
export const lootCurrency = async (data) => {
    ...
    if (game.user.isGM) {
        ...
        const containerItem = game.items.get(data.containerItemId);
        const containerFlags = duplicate(containerItem.getFlag('pick-up-stix', 'pick-up-stix'));
    ...
    if (game.user.isGM) {
        ...
        const newItemDatas = itemDatas.reduce((acc, itemData) => {
            ...
            for (let i = 0; i < actualQty; i++) {
                datas.push(mergeObject(itemData, { data: { [getQuantityDataPath()]: 1 } }));
            }
...
//# sourceMappingURL=../../maps/module/pick-up-stix/main.js.map
